...
app.get("/", (req, res) =>{
    res.render("login", { showAlert: false }); // Pass showAlert as false by default
});
...
app.post("/signup", async (req, res) => {
    ...
    if(!existingUserName && !existingUserEmail) res.render("login", { showAlert: false });
});
...
app.post("/login", async (req, res) => {
    try {
        ...
        if (!checkName || !checkEmail) {
            res.render("login", { showAlert: true }); // Pass showAlert as true if user not found
